...
function debruit=débruitage(matrice)
...
for i = 1:SizeX,
    for j = 1:SizeY
        if i == 1 | i == SizeX | y == 1 | y == SizeY then
            debruit(i,j) = matrice(i,j);
        else 
            debruit(i,j) = 1/*(matrice(i-1,j-1)+matrice(i,j-1)+matrice(i+1,j-1)+matrice(i-1,j)+matrice(i,j)+matrice(i+1,j)+matrice(i-1,j+1)+matrice(i,j+1)+matrice(i+1,j+1))/9;*/        end,
    end;        
end;
